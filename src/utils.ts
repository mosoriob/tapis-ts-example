...
import * as Authenticator from "@tapis/tapis-typescript-authenticator";
...
export const getToken =
  async (): Promise<Authenticator.NewAccessTokenResponse> => {
    ...
    const configuration: Authenticator.Configuration =
      new Authenticator.Configuration(configurationParameters);
    const api: Authenticator.TokensApi = new Authenticator.TokensApi(
      configuration
    );
    ...
    const response: Authenticator.RespCreateToken = await api.createToken(
      createTokenRequest
    );
