...
(async function () {
  try {
    ...
    const configuration: Streams.Configuration = new Streams.Configuration(
      configurationParameters
    );
    //APIs
    const healthApi: Streams.HealthApi = new Streams.HealthApi(configuration);
    const projectApi: Streams.ProjectsApi = new Streams.ProjectsApi(
      configuration
    );
    const siteApi: Streams.SitesApi = new Streams.SitesApi(configuration);
    const instrumentApi: Streams.InstrumentsApi = new Streams.InstrumentsApi(
      configuration
    );
    const variableApi: Streams.VariablesApi = new Streams.VariablesApi(
      configuration
    );
    const measurementApi: Streams.MeasurementsApi = new Streams.MeasurementsApi(
      configuration
    );
    const ontologyApi: Streams.OntologiesApi = new Streams.OntologiesApi(
      configuration
    );
    const channelApi: Streams.ChannelsApi = new Streams.ChannelsApi(
      configuration
    );
    const templateApi: Streams.TemplatesApi = new Streams.TemplatesApi(
      configuration
    );
    const unitApi: Streams.UnitsApi = new Streams.UnitsApi(configuration);
    const measuredPropertyApi: Streams.MeasuredPropertiesApi =
      new Streams.MeasuredPropertiesApi(configuration);
    ...
    const projectResponse: Streams.RespListProjects =
      await projectApi.listProjects(listProjectParams);
    ...
    const siteResponse: Streams.RespListSites = await siteApi.listSites(
      listSiteParams
    );
    ...
    const instrumentResponse: Streams.RespListInstruments =
      await instrumentApi.listInstruments(listInstrumentParams);
    ...
    const variableResponse: Streams.RespListVariables =
      await variableApi.listVariables(listVariableParams);
    ...
    const measurementResponse: Streams.RespListMeasurements =
      await measurementApi.listMeasurements(listMeasurementParams);
    ...
    const measurementDownloadResponse: Streams.RespDownloadMeasurements =
      await measurementApi.downloadMeasurements(downloadMeasurementParams);
    const measurementsDownload: Streams.Measurements =
      measurementDownloadResponse.result;
