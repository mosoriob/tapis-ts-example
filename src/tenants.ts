...
(async function () {
  try {
    ...
    const configuration: Tenants.Configuration = new Tenants.Configuration(
      configurationParameters
    );

    // Create the service with the configuration
    const tenantsApi: Tenants.TenantsApi = new Tenants.TenantsApi(
      configuration
    );
    ...
    // Make the request
    const response: Tenants.RespListTenants = await tenantsApi.listTenants(
      listTenantsRequest
    );
